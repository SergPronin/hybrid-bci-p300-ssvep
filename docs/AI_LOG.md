...
### Fix: зависание «Ожидание протокола / Подготовка протокола»
- Причина: стимулятор сам проходил 3 калибровки, потом ждал `stim_control`, а протокол ждал новый trial — deadlock.
- Все P300 (калибровка + main) только через `stim_control`; `--auto-plan-trials 0`; протокол шлёт `trial` при входе в калибровку и после каждого прогона.

### Порядок main: три блока, shuffle только между ними (2026-05-21)
- Не 45 перемешанных trial: подряд P300×15, ССВП cont×15, ССВП burst×15; seed = порядок 3 блоков (6 перестановок).
- Калибровка по-прежнему всегда в начале (до main).

### Консоль: итоги P300 и MSI ССВП (2026-05-21)
- После каждого P300: плитка-цель, AUC/шаблон, верно/неверно, метрики по плиткам.
- ССВП: Coef MSI по лампам во время блока (~2 с) и полный итог в конце блока.

### Перезапуск PsychoPy перед P300 после ССВП (2026-05-21)
- При shuffle P300 идёт после ССВП; GUI закрывал run_app на ССВП → зависание «Ожидание протокола».
- `_ensure_stimulus_for_p300()` перезапускает стимулятор в `main`/`p300_calib`, пока `stim_control` ждёт trial.

### P300 оверлей как 3a0a997 (2026-05-21)
- Убраны баннер/подсветка плиток; восстановлен серый fullscreen + «Смотрите на плитку: N».
- `stim_control` сохранён; ожидание протокола — тот же серый экран.
- ССВП без изменений: `SsvepCueOverlay`.

### Протокол v2 поверх рабочей базы (2026-05-21)
- Восстановлен FSM `p300_calib` → `main` (45 shuffle) из `cbd4950` + GUI v2 (`spin_calib_*`, `spin_p300_main`, seed).
- PsychoPy: `stim_control.json`, подсказки в том же окне (баннер), без PyQt поверх плиток.
- SSVEP: только `SsvepCueOverlay` при закрытом PsychoPy.
- Запуск: `python scripts/protocol_runner_gui.py`, Start → stim + протокол в одной `session_dir`.

### Откат UI/протокола к 3a0a997 (рабочая база)
- Восстановлены из коммита `3a0a997`: `gui/gui.py`, `protocol_runner.py`, `protocol_runner_gui.py`, `run_app.py`, `app/main.py`.
- Убраны эксперименты v2 (stim_control, лишние PyQt-оверлеи, shuffle FSM) — из-за них ломались окна и зависания.
- В GUI: калибровка по умолчанию **3 прогона** (`spin_plan_trials=3`), основной блок как в 3a0a997: 15 AUC + 15 template + SSVEP.
- `unified_logger.experiments.ndjson` оставлен в логгере, но текущий runner пишет как в 3a0a997.
...
### Полный откат оверлеев/протокола к 8d20668 (2026-05-21)
- `protocol_runner_gui.py`, `protocol_runner.py`, `experiment_queue.py`, `ssvep_cue_overlay.py` — из коммита 8d20668.
- Импорт частот: `lamp_frequencies` (без PyQt6 из ssvep_analyzer).
- GUI: закрыть PsychoPy до оверлея; скрыть окно оператора на время подсказки ССВП.

### Оверлеи как в 8d20668 (2026-05-21)
- `ssvep_cue_overlay.py` и GUI: снова **PyQt5**, только `SsvepCueOverlay` (стрелки ←→↑↓).
- Убран `ProtocolInstructionOverlay` из `protocol_runner_gui` (его не было в 8d20668).
- P300-подсказки — на мониторе **PsychoPy** (stim_control), не Qt-оверлей.

### protocol_runner_gui: PyQt6, без импорта ssvep_analyzer (2026-05-21)
- Причина падения: PyQt5 + PyQt6 в одном процессе (импорт `SSVEPAnalyzerWindow`).
- GUI протокола и оверлеи → PyQt6; частоты ламп → `ssvep_analysis/lamp_frequencies.py`.

### Сохранение данных по экспериментам (2026-05-21)
- Каждый trial P300 / блок SSVEP → `experiment_runs/.../experiments/exp_NNNNN/`
- `experiment.json`: номер, цель, итоги (AUC + шаблон / MSI coef по лампам), ссылки на артефакты
- `eeg.npz`, `markers.ndjson`, `events.ndjson` — срез сессии на этот эксперимент
- Сводка: `experiments.ndjson`, `experiments.csv`; полное EEG сессии — `eeg.npz` в корне
- Схема лога: `hybrid_protocol/v2`

### GUI: компактный режим + монитор ЭЭГ (2026-05-21)
- По умолчанию: поток LSL, «Подключить»/«Отключить», график pyqtgraph, Старт/Стоп.
- «Развернуть настройки» — полная форма (протокол, MSI, каналы, COM).
- При подключении — живая осциллограмма канала 1.

### GUI: MSI-параметры и прокрутка каналов (2026-05-21)
- `protocol_runner_gui.py`: блок MSI как в `ssvep_analyzer` (Fs, отсчёты, окно с), подстановка из LSL при выборе ЭЭГ.
- Каналы P300/ССВП в `QScrollArea` (max 200 px) — виден 21-й канал.
- Протокол использует `ssvep_fs_hz` / `ssvep_window_sec` из GUI, не перезаписывает Fs потоком при MSI.

### P300: калибровка в одном блоке с main (2026-05-21)
- Убрана отдельная фаза FSM `p300_calib`; после preflight сразу `main`.
- `build_main_queue`: сегмент P300 = `p300_calib_trials` (5) + `p300_main_trials` (15) подряд; очередь **50** trial.
- Между trial внутри блока P300 паузы нет; `calibration_done` после последнего calib trial.
- Эпохи P300 между calib trial не сбрасываются (шаблон); `stim_control` — индекс внутри блока P300.
- Тесты: `test_experiment_queue.py`, `test_protocol_runner_fsm.py` — OK.

## 2026-05-21 — Исправление оверлея ССВП

**Проблема:** SsvepCueOverlay не появлялся (серый экран без стрелок).
**Причина:** После предыдущих сессий в `_sync_ssvep_cue_overlay` были добавлены вызовы `self.hide()` на главном окне перед показом оверлея. На Windows (и macOS) `Qt.Tool`-окно является дочерним по отношению к главному — когда главное окно скрывается, Tool-окно тоже исчезает.
**Исправление:** Удалены `self.hide()`, `raise_()` и `activateWindow()` из `_sync_ssvep_cue_overlay` и `_dismiss_ssvep_overlay` — логика возвращена дословно к коммиту `8d20668`. Единственное сохранённое отличие — импорт `lamp_frequency_choices` из `ssvep_analysis/lamp_frequencies.py` (для избегания PyQt5/PyQt6 конфликта).
**Risk:** LOW — изменён только GUI-метод синхронизации оверлея.

## 2026-05-21 — Компактный GUI, срез EEG/маркеров, объединение calib+main

### Компактный GUI
- По умолчанию: поле LSL + «Подключить»/«Отключить», живой осциллограф (pyqtgraph, канал 1), кнопки Запустить/Остановить.
- «⚙ Развернуть настройки» показывает полную форму (subject, output, протокол, стимулятор, мигалка, каналы P300/SSVEP).
- При клике «Подключить» inlet остаётся открытым, таймер 80 мс обновляет график; «Отключить» — закрывает.

### Срез EEG и маркеры на эксперимент
- `_pull_lsl()`: для каждого маркера вызывается `logger.record_marker(lsl_time, sample)`.
- При детекте `trial_start` (tid != None): `logger.mark_experiment_start(kind="p300", phase, cue_target_tile_id)`.
- `_start_ssvep_block()`: `logger.mark_experiment_start(kind="ssvep", mode, target_lamp_0idx)`.
- Теперь `exp_NNNNN/eeg.npz` и `markers.ndjson` содержат корректный срез сессии на каждый trial/block.

### P300 калибровка + main в одном блоке (FSM)
- Убрана отдельная фаза `P300Calib`: после preflight сразу `_begin_main_phase()`.
- `_begin_main_phase()` prepends `n_calib` QueueItem(phase="calib") перед основной очередью.
- `_run_main_queue_item()` определяет фазу через `item.phase`, логирует `calibration_done` после последнего calib-trial.
- Template building: переключено с проверки `state==P300Calib` на `not _template_locked` в Main-состоянии.
- `QueueItem` получил поле `phase: str = "main"`.
- Тесты: `test_experiment_queue.py`, `test_protocol_logger.py` — OK. Risk: MEDIUM (FSM упрощён).
